...
(function() {
        ...
        if (!gameMode) {
            container.innerHTML = `
                <div style="text-align: center;">
                    ...
                    <div style="margin: 30px 0;">
                        <button onclick="window.showGameModes()" style="background: #5a3a2a; color: white; border: none; padding: 15px 40px; margin: 10px; border-radius: 8px; font-size: 1.2em; cursor: pointer;">🎮 ИГРАТЬ</button>
                        <button onclick="window.showRatings()" style="background: #5a3a2a; color: white; border: none; padding: 15px 40px; margin: 10px; border-radius: 8px; font-size: 1.2em; cursor: pointer;">🏆 РЕЙТИНГ</button>
                    </div>
                </div>
            `;
            ...
            container.innerHTML = `
                <div style="text-align: center;">
                    ...
                    <div style="margin: 30px 0;">
                        <button onclick="window.startGameMode('classic', 4)" style="background: #27ae60; color: white; border: none; padding: 15px 40px; margin: 10px; border-radius: 8px; font-size: 1.2em; cursor: pointer;">📊 КЛАССИЧЕСКИЙ (4x4)</button>
                        <button onclick="window.startGameMode('classic', 8)" style="background: #27ae60; color: white; border: none; padding: 15px 40px; margin: 10px; border-radius: 8px; font-size: 1.2em; cursor: pointer;">📊 КЛАССИЧЕСКИЙ (8x8)</button>
                    </div>
                    
                    <div style="margin-top: 20px;">
                        <button onclick="window.backToMenu()" style="background: #8b4513; color: white; border: none; padding: 10px 25px; border-radius: 6px; cursor: pointer;">⬅ НАЗАД</button>
                    </div>
                </div>
            `;
            ...
            container.innerHTML = `
                <div style="text-align: center;">
                    ...
                    <div style="margin-top: 20px;">
                        <button onclick="window.resetMemoryGame()" style="background: #8b4513; color: white; border: none; padding: 10px 25px; border-radius: 6px; font-size: 1em; cursor: pointer; margin-right: 10px;">🔄 НОВАЯ ИГРА</button>
                        <button onclick="window.backToMenu()" style="background: #5a3a2a; color: white; border: none; padding: 10px 25px; border-radius: 6px; font-size: 1em; cursor: pointer;">🏠 ГЛАВНОЕ МЕНЮ</button>
                    </div>
                </div>
            `;
            renderMemoryGrid();
    ...
    window.startGameMode = function(mode, size) {
        gameMode = 'playing';
        gridSize = size;
    ...
    function renderMemoryGrid() {
        ...
        for (let i = 0; i < totalCards; i++) {
            ...
            if (!matched[i] && !locked && gameActive) {
                card.onclick = (function(idx) { return function() { window.flipCard(idx); }; })(i);
            } else {
                card.onclick = null;
            }
            ...
            if (matchesFound === totalPairs) {
                ...
                if (container) {
                    ...
                    statusDiv.innerHTML = `
                        ...
                        <button onclick="this.parentElement.remove(); window.resetMemoryGame()" style="background: #8b4513; color: white; border: none; padding: 10px 25px; border-radius: 6px; font-size: 1em; cursor: pointer; margin-top: 15px;">ИГРАТЬ СНОВА</button>
                        <button onclick="this.parentElement.remove(); window.backToMenu()" style="background: #5a3a2a; color: white; border: none; padding: 10px 25px; border-radius: 6px; font-size: 1em; cursor: pointer; margin-top: 15px; margin-left: 10px;">ГЛАВНОЕ МЕНЮ</button>
                    `;
